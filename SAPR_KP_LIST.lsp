...
(defun plot_side (pd / pl1 pl2 pl3)
 	(setq pt1 (list (car pd) (cadr pd))
        ...
        pd6(list (+ (car pd1) (/ (nth 5 other) 2.0))
         ...
         ptr4y(list (car pd4)
               (+ (cadr pd4) 0.01))
         ptr4x(list (+ (car pd4) 0.02)
               (cadr pd4))
         ptr5y(list (- (car pd5) 0.01)
               (cadr pd5))
          ptr5x(list (car pd5)
               (- (cadr pd5) 0.01))
  ...
  (setq cside (ssadd (entlast) cside))
  (command "line" pd3 pd4 "")
  (setq cside (ssadd (entlast) cside))
  (command "line" pd4 pd5 "")
  (setq cside (ssadd (entlast) cside))
  (command "line" pd5 pd2 "")
  (setq cside (ssadd (entlast) cside))
  (command "zoom" "w" pwzs1 pwzs2)
  (command "trim" "o" "q" "" ptr3x ptr2x ptr4x ptr5x"")
  (setq cside (ssadd (entlast) cside))
  (command "zoom" "p")
 ...
 (if (= (substr (nth 5 plot_data) 1 1) "y")
   (progn
      ...
      (command "dimtoh" "on")
      (command "dimscale" (* (nth 0 other) 0.04))
      (command "dimlinear" pd1 pd1b pt1)
     (setq cside (ssadd (entlast) cside))
